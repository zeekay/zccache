...
pub(super) fn paths_to_prost(paths: &[crate::core::NormalizedPath]) -> Vec<zccache_v1::Path> {
    paths.iter().map(path_to_prost).collect()
}

pub(super) fn paths_from_prost(paths: Vec<zccache_v1::Path>) -> Vec<crate::core::NormalizedPath> {
    paths.into_iter().map(path_from_prost).collect()
}

pub(super) fn private_daemon_session_options_to_prost(
    options: &crate::protocol::PrivateDaemonSessionOptions,
...
pub(super) fn artifact_data_to_prost(
    artifact: &crate::protocol::ArtifactData,
...
) -> Result<crate::protocol::ArtifactData, String> {
    Ok(crate::protocol::ArtifactData {
        outputs: artifact
            .outputs
            .into_iter()
...
pub(super) fn artifact_output_to_prost(
    output: &crate::protocol::ArtifactOutput,
...
) -> Result<crate::protocol::ArtifactOutput, String> {
    Ok(crate::protocol::ArtifactOutput {
...
fn artifact_payload_to_prost(
    payload: &crate::protocol::ArtifactPayload,
) -> zccache_v1::ArtifactPayload {
    use zccache_v1::artifact_payload::Body;

    zccache_v1::ArtifactPayload {
        body: Some(match payload {
            crate::protocol::ArtifactPayload::Bytes(bytes) => Body::Bytes(bytes.as_ref().clone()),
            crate::protocol::ArtifactPayload::Path(path) => Body::Path(path_to_prost(path)),
        }),
    }
}
...
) -> Result<crate::protocol::ArtifactPayload, String> {
    use zccache_v1::artifact_payload::Body;

    match payload.body {
        Some(Body::Bytes(bytes)) => Ok(crate::protocol::ArtifactPayload::Bytes(
            std::sync::Arc::new(bytes),
        )),
        Some(Body::Path(path)) => Ok(crate::protocol::ArtifactPayload::Path(path_from_prost(
            path,
        ))),
...
pub(super) fn lookup_result_to_prost(
    result: &crate::protocol::LookupResult,
) -> zccache_v1::LookupResult {
    use zccache_v1::lookup_result::Body;

    zccache_v1::LookupResult {
        body: Some(match result {
            crate::protocol::LookupResult::Hit { artifact } => {
                Body::Hit(artifact_data_to_prost(artifact))
            }
            crate::protocol::LookupResult::Miss => Body::Miss(zccache_v1::Empty {}),
        }),
    }
}
...
) -> Result<crate::protocol::LookupResult, String> {
    use zccache_v1::lookup_result::Body;

    match result.body {
        Some(Body::Hit(artifact)) => Ok(crate::protocol::LookupResult::Hit {
            artifact: artifact_data_from_prost(artifact)?,
        }),
        Some(Body::Miss(_)) => Ok(crate::protocol::LookupResult::Miss),
        None => Err("missing required v16 prost field LookupResult.body".to_string()),
    }
}

pub(super) fn store_result_kind_to_prost(
    result: &crate::protocol::StoreResult,
) -> zccache_v1::StoreResultKind {
    match result {
        crate::protocol::StoreResult::Stored => zccache_v1::StoreResultKind::Stored,
        crate::protocol::StoreResult::AlreadyExists => zccache_v1::StoreResultKind::AlreadyExists,
    }
}
...
pub(super) fn store_result_kind_from_prost(
    ...
    match zccache_v1::StoreResultKind::try_from(kind) {
        ...
        Ok(zccache_v1::StoreResultKind::AlreadyExists) => {
            Ok(crate::protocol::StoreResult::AlreadyExists)
        }
...
pub(super) fn exec_output_streams_to_prost(
    streams: crate::protocol::ExecOutputStreams,
...
) -> crate::protocol::ExecOutputStreams {
    crate::protocol::ExecOutputStreams {
        stdout: streams.stdout,
        stderr: streams.stderr,
    }
...
pub(super) fn exec_cache_policy_to_prost(
    policy: crate::protocol::ExecCachePolicy,
) -> zccache_v1::ExecCachePolicy {
    match policy {
        crate::protocol::ExecCachePolicy::Normal => zccache_v1::ExecCachePolicy::Normal,
        crate::protocol::ExecCachePolicy::Bypass => zccache_v1::ExecCachePolicy::Bypass,
        crate::protocol::ExecCachePolicy::ReadOnly => zccache_v1::ExecCachePolicy::ReadOnly,
    }
}

pub(super) fn exec_cache_policy_from_prost(
    policy: i32,
) -> Result<crate::protocol::ExecCachePolicy, String> {
    match zccache_v1::ExecCachePolicy::try_from(policy) {
        Ok(zccache_v1::ExecCachePolicy::Normal) => Ok(crate::protocol::ExecCachePolicy::Normal),
        Ok(zccache_v1::ExecCachePolicy::Bypass) => Ok(crate::protocol::ExecCachePolicy::Bypass),
        Ok(zccache_v1::ExecCachePolicy::ReadOnly) => Ok(crate::protocol::ExecCachePolicy::ReadOnly),
...
pub(super) fn rust_artifact_info_to_prost(
    info: &crate::protocol::RustArtifactInfo,
...
) -> Result<crate::protocol::RustArtifactInfo, String> {
    Ok(crate::protocol::RustArtifactInfo {
...
pub(super) fn session_stats_to_prost(
...
pub(super) fn session_stats_from_prost(
    stats: zccache_v1::SessionStats,
...
fn phase_profile_to_prost(
    profile: &crate::protocol::PhaseProfileSummary,
...
pub(super) fn daemon_status_to_prost(
...
) -> Result<crate::protocol::DaemonStatus, String> {
    Ok(crate::protocol::DaemonStatus {
...
fn private_daemon_status_to_prost(
    status: &crate::protocol::PrivateDaemonStatus,
...
) -> crate::protocol::PrivateDaemonStatus {
    crate::protocol::PrivateDaemonStatus {
        enabled: status.enabled,
        owners: status
            .owners
            .into_iter()
            .map(|owner| crate::protocol::PrivateDaemonOwnerStatus {
                pid: owner.pid,
                ref_count: owner.ref_count,
            })
            .collect(),
...
pub(super) fn path_to_prost(path: &crate::core::NormalizedPath) -> zccache_v1::Path {
    zccache_v1::Path {
        value: path.as_path().to_string_lossy().into_owned(),
    }
}

pub(super) fn path_from_prost(path: zccache_v1::Path) -> crate::core::NormalizedPath {
