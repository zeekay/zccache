...
fn print_phase_profile(profile: &zccache_monocrate::daemon::ProfileSnapshot) {
    let wide = "=".repeat(80);
    let dash = "-".repeat(70);
...
async fn cold_path_stress_profile() {
    ...
    let file_counts = [50];
    let mut all_results: Vec<(usize, ColdPassResult, zccache_monocrate::daemon::ProfileSnapshot)> = Vec::new();
