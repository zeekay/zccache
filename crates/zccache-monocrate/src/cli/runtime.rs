...
//!
//! Extracted from `cli/mod.rs` in wave 6 of the monocrate consolidation
...
async fn check_daemon_version(endpoint: &str) -> VersionCheck {
    ...
    if conn
        .send(&zccache_monocrate::protocol::Request::Status)
        .await
        .is_err()
    {
...
async fn stop_stale_daemon(endpoint: &str) {
    if let Ok(mut conn) = connect_client(endpoint).await {
        let _ = conn
            .send(&zccache_monocrate::protocol::Request::Shutdown)
            .await;
...
pub async fn ensure_daemon(endpoint: &str) -> Result<(), String> {
    ...
    spawn_and_wait(
        endpoint,
        zccache_monocrate::core::lifecycle::REASON_INITIAL_START,
    )
    .await
}
