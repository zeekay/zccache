...
//! via [`zccache_monocrate::daemon::trampoline`] before entering [`run_server`].
...
fn main() {
    ...
    if args.foreground {
        ...
        match args.log_file.as_deref() {
            Some(path) => zccache_monocrate::daemon::trampoline::redirect_stdio_to_log(path),
            None => zccache_monocrate::daemon::trampoline::detach_stdio(),
        }
        ...
        zccache_monocrate::daemon::trampoline::unlock_exe();
        zccache_monocrate::daemon::trampoline::release_cwd();
        run_server(args);
    } else {
        print_status(&args);
    }
...
fn run_server(args: Args) {
    ...
    zccache_monocrate::daemon::lifecycle::write_event(
        zccache_monocrate::daemon::lifecycle::EVENT_SPAWN,
        serde_json::json!({
            "endpoint": &endpoint,
    ...
    rt.block_on(async {
        let mut server = match zccache_monocrate::daemon::DaemonServer::bind(&endpoint) {
            Ok(s) => s,
            Err(e) => {
