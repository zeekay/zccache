...
//! the absolute path to that directory. This gives true deduplication and is
//! portable across all OSes without symlinks, which require dev mode or admin
...
/// True if the directory exists and has the `.ready` sentinel, meaning a
...
/// absolute path to the symbol directory plus a trailing newline so `cat`
/// output stays tidy. This is the "no duplicates" half of the placement
/// contract.
