...
pub(crate) async fn cmd_status(endpoint: &str, json: bool) -> ExitCode {
    ...
    if let Err(e) = conn.send(&zccache_monocrate::protocol::Request::Status).await {
    ...
    match recv_result {
        Some(zccache_monocrate::protocol::Response::Status(s)) => {
            if json {
            ...
            println!(
                ...
                zccache_monocrate::protocol::PROTOCOL_VERSION,
                endpoint,
...
fn print_status_ok_json(endpoint: &str, s: &zccache_monocrate::protocol::DaemonStatus) {
    let total = s.cache_hits + s.cache_misses;
    let hit_rate = if total > 0 {
    ...
    let value = serde_json::json!({
        "status": "ok",
        "endpoint": endpoint,
        "protocol_version": zccache_monocrate::protocol::PROTOCOL_VERSION,
        "hit_rate": hit_rate,
