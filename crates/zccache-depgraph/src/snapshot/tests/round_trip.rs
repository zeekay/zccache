...
use crate::graph::{ContextState, DepGraph};
use crate::scanner::{IncludeDirective, IncludeKind, ScanResult};
use crate::search_paths::IncludeSearchPaths;
use crate::snapshot::{load_from_file, save_to_file};
...
fn populated_graph_roundtrip() {
    ...
    let source_hash = zccache_monocrate::hash::hash_bytes(b"source content");
    let header_hash = zccache_monocrate::hash::hash_bytes(b"header content");
    let pch_hash = zccache_monocrate::hash::hash_bytes(b"pch content");
...
fn last_file_hashes_roundtrip() {
    ...
    let hash1 = zccache_monocrate::hash::hash_bytes(b"content1");
    let hash2 = zccache_monocrate::hash::hash_bytes(b"content2");
    let hashes: std::collections::HashMap<NormalizedPath, ContentHash> = [
...
fn artifact_key_some_roundtrip() {
    ...
    let hash = zccache_monocrate::hash::hash_bytes(b"source");
    let hashes: std::collections::HashMap<NormalizedPath, ContentHash> =
...
fn empty_strings_roundtrip() {
    ...
    // Empty path hash.
    let hash = zccache_monocrate::hash::hash_bytes(b"");
    let hashes: std::collections::HashMap<NormalizedPath, ContentHash> =
...
fn file_hash_bytes_exact_roundtrip() {
    ...
    let key = graph.register(make_ctx("/src/a.cpp"));
    let source_hash = zccache_monocrate::hash::hash_bytes(b"specific source content 12345");
    let header_hash = zccache_monocrate::hash::hash_bytes(b"specific header content 67890");
